...
def parse_args():
    ...
    for key, value in args_dict.items():
        args_dict_environ[key] = type_switch(os.getenv(key.upper(), value))
    args_easydict = EasyDict(args_dict_environ)
    return args_easydict


def type_switch(environ_value):
    if isinstance(environ_value, int):
        return int(environ_value)
    elif isinstance(environ_value, float):
        return float(environ_value)
    elif isinstance(environ_value, bool):
